...
export const config: IDashboardConfig = /*return*/ {
  id: "sync_no_service",
  name: "Analytics Dashboard for Sync No Service Bot",
  icon: "dashboard",
  url: "sync_no_service",
  description: "Analytics Dashboard for Sync No Service Bot",
    ...
    },
    {
      ...
      params: {
        ...
        queries: {
          ...
          successRate: {
            query: ({ granularity }) => `
                where name == 'SNSEvent.Exit' or name == 'SNSEvent.Escalation' 
                | summarize value = count() by name  `,
            calculated: (results) => {
              var successExitCount, totalExitCount, escalationCount;
              results.forEach(row => {
                if (row.name == "SNSEvent.Exit") {
                  totalExitCount = row.value;
                }
                if (row.name == "SNSEvent.Escalation") {
                  escalationCount = row.value;
                }
              })
              successExitCount = totalExitCount - escalationCount;
              return { "successRate": _.values([{ name: "Escalation", value: escalationCount }, { name: "Bot resolved issue", value: successExitCount }]) };
            }
          },
          feedback: {
            query: ({ granularity }) => `
            ...
            calculated: (results) => {
              ...
              results.forEach(row => {
                if (row.Column1 == "yes") {
                  ...
                  feedbackResults.push({ name: "Negative", value: row.value })
                }
              });

              return { "feedback": _.values(feedbackResults) };
            }
          },
          timeline: {
            query: ({ granularity }) => `
  ...
  elements: [
    {
      ...
      type: "Timeline",
      title: "Sessions",
      subtitle: "How many sessions with bot were opened",
      size: { w: 4, h: 8 },
      source: "ai:sessions"
    },
    {
      id: "successRate",
      type: "PieData",
      title: "Successful exit vs Esclation",
      subtitle: "How many sessions with bot were opened",
      size: { w: 4, h: 8 },
      dependencies: { values: "ai:successRate" },
    ...
    },
    {
      ...
      type: "PieData",
      title: "Feedback",
      subtitle: "How many users reported that bot was helpful",
      size: { w: 4, h: 8 },
      dependencies: { values: "ai:feedback" },
      props: { showLegend: true }
    },
    {
      id: "map",
