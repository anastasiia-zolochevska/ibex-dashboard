		...
		},
		{
			...
			params: {
                ...
                | summarize value = count() by name  `,
						calculated: (results) => {
              var successRateDisplay = []
              results.forEach(row => {
                successRateDisplay.push({ name: row.name.split(".")[1], value: row.value })
              })

              return { "successRate": _.values(successRateDisplay) };
            }
					},
					feedback: {
						query: ({ granularity }) => `
              ...
              console.log("Ana2", feedbackResults);
